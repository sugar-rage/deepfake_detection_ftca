# =============================================
# Baseline 3D-CNN + Transformer (No Drift)
# =============================================
import os
import numpy as np
import torch
...
# ===============================
# Baseline 3D Transformer (No Drift)
# ===============================
class Baseline3DTransformer(nn.Module):
    def __init__(self, num_classes=1):
        super().__init__()

        # Input: Only RGB → 3 channels
        self.layer1 = ResidualBlock3D(3, 32)
...
def train_model(dataset_path, device, epochs=25, batch_size=4, lr=1e-4):
    ...
    model = Baseline3DTransformer().to(device)
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), f"{dataset_path}/baseline_no_drift_best.pt")
            print("💾 Saved new best baseline model!")

    print("✅ Training Completed (No Drift).")
    return model
...
if __name__ == "__main__":
    ...
    evaluate_model(dataset_path, device, f"{dataset_path}/baseline_no_drift_best.pt")
